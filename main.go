...
// config defines how tag should be modified
type config struct {
	file     string
	output   string
	quiet    bool
...
func realMain() error {
	...
	rewriteNode, errs := cfg.rewrite(node, start, end)
	if errs != nil {
	...
	out, err := cfg.format(rewriteNode, errs)
...
// parseConfig parse the config into config
func parseConfig(args []string) (*config, error) {
	var (
		// file flag
		flagFile  = flag.String("file", "", "Filename to be parsed")
		flagWrite = flag.Bool("w", false, "Write results to (source) file")
		flagQuite = flag.Bool("quiet", false, "Don't print result to stdout")
		...
		flagOffset = flag.Int("offset", 0,
			"Byte offset of the cursor position inside a struct."+
				"Can be an where from the comment until closing bracket")
		flagLine   = flag.String("line", "", "line number of the field or a range of line. i.e: 4 or 8")
		flagStruct = flag.String("struct", "", "Strcut name to be processed")
		...
		flagAddTags    = flag.String("add-tags", "", "Adds tags for the comma separated list of keys."+
		...
		flagTransform            = flag.String("transform", "snakecase", "Transform adds a transform rule when adding tags."+
		...
		// formatting
		flagFormatting = flag.String("template", "", "Format the given tag's value. i.e:\"column:{field}\", \"field_name:{field}\" ")

		// option flags
		flagRemoveOptions = flag.String("remove-options", "", "Remove the comma separated list of options from the given keys, "+
			"i.e: json=omitempty,hcl=squash")
		flagClearOptions = flag.Bool("clear-options", false, "Clear all tag options")
		flagAddOptions   = flag.String("add-options", "", "Add the options pere given key. i.e: json=omitempty,hcl=squash")
	)
	...
	cfg := &config{
		...
		skipUnexpectedFields: *flagSkipUnexportedFields,
	}
...
// findSelection returns the start and end position of the field that are suspect to change.
...
func (c *config) lineSelection(file ast.Node) (int, int, error) {
	...
	if start > end {
		return 0, 0, errors.New("wrong range, start line cannot larger than end line")
	}
	return start, end, nil
}
...
func (c *config) structSelection(file ast.Node) (int, int, error) {
	...
	if encStruct == nil {
		return 0, 0, errors.New("struct name doesn't exist")
	}
...
// collectStructs collects and map structType nodes to their positions
...
// allSelection select all structs in a file
...
// isPublicName judge the string is prefix with a upper char
...
// clear all tag options in tags
func (c *config) clearOptions(tags *stag.Tags) *stag.Tags {
	if !c.clearOption {
		return tags
	}
...
func (c *config) clearTags(tags *stag.Tags) *stag.Tags {
	if !c.clear {
		return tags
	}
...
func (c *config) removeTagOptions(tags *stag.Tags) (*stag.Tags, error) {
...
func (c *config) addTagOptions(tags *stag.Tags) (*stag.Tags, error) {
...
func (c *config) addTags(fieldName string, tags *stag.Tags) (*stag.Tags, error) {
...
// remove tags in config
func (c *config) removeTags(tags *stag.Tags) *stag.Tags {
...
func (c *config) rewrite(node ast.Node, start, end int) (ast.Node, error) {
	...
	rewriteFunc := func(n ast.Node) bool {
		...
		for _, f := range x.Fields.List {
			...
			if len(f.Names) != 0 {
				for _, field := range f.Names {
					if !c.skipUnexpectedFields || isPublicName(field.Name) {
			...
			if f.Names == nil {
				...
				if !c.skipUnexpectedFields {
					fieldName = ident.Name
				}
			}

			// noting to process continue with next line
			if fieldName == "" {
				continue
			}
...
// validate check the config is valid or not
func (c *config) validate() error {
	...
	if (c.add == nil || len(c.add) == 0) &&
		...
		!c.clear && !c.clearOption &&
		(c.removeOptions == nil || len(c.removeOptions) == 0) &&
		(c.remove == nil || len(c.remove) == 0) {
		return errors.New("one of [-add-tags, -add-options, -remove-tags, -remove-options, -clear-tags, -clear-options]" +
			"should be defined")
	}
...
// Error return the string contents all the errors in the rewriteError
...
// Append add a error to rewriteErrors
...
// parseLines parse the given buffer and returns a slice of lines
func parseLines(buf io.Reader) ([]string, error) {
	var lines []string
	scanner := bufio.NewScanner(buf)
	for scanner.Scan() {
		txt := scanner.Text()
		// check for any line directive and store it for next iteration to re-construct the original file.
		// If it's not a line directive, continue constructing the original file
	...
	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("invalid scanner input1: %s", err)
	}
	return lines, nil
}
...
// Note: this only splits the line directive that the go.Parser outputs.
// If the go parser changes the format of the line directive, make sure to fix it in the below function
func split(line string) (int, error) {
	...
	return 0, fmt.Errorf("cannot parse line: '%s'", line)
}

// deref takes an expression, and removes all its leading "*" and "[]"
// operator. Uuse case: if found expression is a "*t" or "[]t", we need to check if "t" contents a struct expression
...
func (c *config) format(file ast.Node, rwErrs error) (string, error) {
	...
	case "json":
		// print first the whole file and then cut out our selection. the reason we don't directly print the struct that
		// the printer is not capable of printing loosy comments
		var buf bytes.Buffer

		// this is the default config from `format.Node()` but we add `printer.SourcePos` to get the original source position of the modified lines
